...
<div class="container mt-5 pt-4 mb-5">
    <div class="row mb-5 align-items-center bg-white p-4 p-md-5 shadow-sm" style="border-radius: 24px; border: 1px solid rgba(0,0,0,0.05);">
        ...
        <div class="col-md">
            <span class="badge mb-1 text-white px-3 py-1 rounded-pill" style="background-color: #f53003; font-size: 0.75rem; font-weight: 700; letter-spacing: 0.5px;">COMUNIDAD MERCAZONE</span>
            <h1 class="fw-bolder text-dark mb-1" style="letter-spacing: -0.5px;">{{ $negocio->nombre_negocio }}</h1>
            <p class="text-secondary mb-0"><i class="bi bi-info-circle me-2 text-muted"></i>Espacio oficial de consultas e información de stock en tiempo real.</p>
        </div>
        <div class="col-md-auto mt-3 mt-md-0 d-flex gap-2 flex-wrap">
            ...
            <a href="{{ route('foros.index') }}" class="btn rounded-pill px-4 fw-bold shadow-sm text-white" style="background-color: #f53003; border: none;">
                <i class="bi bi-grid-fill me-2"></i>Ver todos los foros
            </a>
        </div>
    </div>
        ...
        <div class="col-12">
            ...
            <div class="d-flex justify-content-between align-items-center mb-4">
                <h3 class="fw-bolder text-dark mb-0" style="letter-spacing: -0.5px;">
                    <i class="bi bi-chat-left-text-fill me-2" style="color: #f53003;"></i>Conversaciones de la comunidad
                </h3>
                <a href="#nueva-pregunta-seccion" class="btn btn-light rounded-pill fw-bold text-dark px-3 border shadow-sm">
                    <i class="bi bi-plus-lg me-1"></i> Hacer una pregunta
                </a>
            </div>
                ...
                <div class="card mb-4 border-0 shadow-sm" style="border-radius: 24px;">
                    <div class="card-body p-4 p-md-5">
                        ...
                        <div class="d-flex justify-content-between align-items-start mb-3">
                            <div class="d-flex align-items-center">
                                <div class="text-white rounded-circle d-flex justify-content-center align-items-center fw-bold me-3 shadow-sm" 
                                     style="width: 52px; height: 52px; background-color: #f53003; font-size: 1.2rem;">
                                    {{ strtoupper(substr($mensaje->usuario->nombre, 0, 1)) }}
                                </div>
                                <div>
                                    <h5 class="mb-0 fw-bold text-dark">{{ $mensaje->usuario->nombre }}</h5>
                            ...
                            @if($mensaje->user_id === auth()->id())
                                <span class="badge bg-light text-dark border border-2 rounded-pill px-3 py-2 fw-bold" style="font-size: 0.75rem;">Tu pregunta</span>
                            @endif
                        </div>
                        ...
                        @if($mensaje->respuestas->count() > 0)
                            <div class="ps-4 border-start border-3 mt-4" style="border-color: rgba(245, 48, 3, 0.15) !important;">
                                <div class="text-muted small fw-bold mb-3 text-uppercase tracking-wider" style="font-size: 0.75rem;">Respuestas</div>
                                
                                @foreach($mensaje->respuestas as $respuesta)
                                    @php $esComerciante = ($respuesta->user_id === $negocio->user_id); @endphp
                                    ...
                                    <div class="d-flex mb-3 align-items-start" style="max-width: 950px;">
                                        <div class="rounded-circle d-flex justify-content-center align-items-center fw-bold me-3 shadow-sm flex-shrink-0" 
                                             style="width: 40px; height: 40px; font-size: 0.9rem; {{ $esComerciante ? 'background-color: #2e7d32; color: white;' : 'background-color: #f8f9fa; color: #212529;' }}">
                                            @if($esComerciante)
                                        ...
                                        <div class="p-3 px-4 rounded-4 flex-grow-1 shadow-sm" 
                                             style="border-top-left-radius: 0px; {{ $esComerciante ? 'background-color: #e8f3ee; border: 1px solid rgba(46, 125, 50, 0.1);' : 'background-color: #f8f9fa; border: 1px solid rgba(0,0,0,0.03);' }}">
                                            <div class="d-flex justify-content-between align-items-center mb-1">
                                                <span class="fw-bold text-dark">
                                                    {{ $respuesta->usuario->nombre }} 
                                                    @if($esComerciante)
                                                        <span class="badge rounded-pill bg-success ms-1 text-white fw-bold" style="font-size: 0.65rem; padding: 3px 7px;">Comerciante Vendedor</span>
                                                    @endif
                                                </span>
                                                <small class="text-muted" style="font-size: 0.75rem;">{{ $respuesta->created_at->diffForHumans() }}</small>
                                            </div>
                                            <p class="text-secondary mb-0 mt-1" style="white-space: pre-line; line-height: 1.5; font-size: 0.95rem;">{{ $respuesta->contenido }}</p>
                                        </div>
                                    </div>
                                @endforeach
                            </div>
                        ...
                        <div class="mt-4">
                            <button class="btn btn-link p-0 text-decoration-none fw-bold small d-flex align-items-center" 
                                ...
                                <i class="bi bi-reply-all-fill fs-5 me-1"></i> Responder a este hilo
                            </button>
                            ...
                            <div class="collapse mt-3" id="replyForm{{ $mensaje->id }}">
                                <form action="{{ route('foros.store') }}" method="POST" class="p-3 bg-light rounded-4 border" style="max-width: 700px;">
                                    ...
                                    <textarea name="contenido" class="form-control border-0 bg-white py-3 mb-2 shadow-sm" rows="2" placeholder="Escribe tu respuesta aquí..." required style="border-radius: 12px; font-size: 0.95rem;"></textarea>
                                    <div class="text-end">
                                        <button type="submit" class="btn btn-sm px-4 py-2 fw-bold text-white shadow-sm" style="background-color: #f53003; border-radius: 12px;">
                                            Enviar respuesta <i class="bi bi-send-fill small ms-1"></i>
                                        </button>
                                    </div>
                                </form>
                            </div>
            ...
            @empty
                <div class="text-center py-5 bg-white shadow-sm mb-5" style="border-radius: 24px; border: 1px solid rgba(0,0,0,0.05);">
                    ...
                    <p class="text-secondary fw-bold fs-5 mb-1">Aún no hay preguntas en este puesto</p>
                    <p class="text-muted small">Sé la primera persona en escribir utilizando el formulario de abajo.</p>
                </div>
            @endforelse
            ...
            <div class="card border-0 shadow-sm col-xl-8 mx-auto" style="border-radius: 24px;">
                <div class="card-body p-4 p-md-5">
                    <div class="d-flex align-items-center mb-3">
                        ...
                        <div>
                            <h4 class="fw-bolder text-dark mb-0" style="letter-spacing: -0.5px;">Publicar una nueva consulta</h4>
                            <p class="text-muted small mb-0">Tu duda será visible para todos los usuarios y el comerciante.</p>
                        </div>
                    </div>
                    ...
                    <form action="{{ route('foros.store') }}" method="POST">
                        ...
                        <div class="mb-3">
                            <label for="titulo" class="form-label fw-bold text-dark small">Asunto o tema principal</label>
                            <input type="text" class="form-control bg-light border-0 py-3 px-3" id="titulo" name="titulo" placeholder="Ej: ¿Tendréis stock de cerezas este fin de semana?" required style="border-radius: 14px; font-size: 0.95rem;">
                        </div>

                        <div class="mb-4">
                            <label for="contenido" class="form-label fw-bold text-dark small">Explica detalladamente tu duda</label>
                            <textarea class="form-control bg-light border-0 py-3 px-3" id="contenido" name="contenido" rows="4" placeholder="Escribe aquí los detalles de tu consulta para que el vendedor pueda ayudarte mejor..." required style="border-radius: 14px; font-size: 0.95rem;"></textarea>
                        </div>
                        ...
                        <div class="text-end">
                            <button type="submit" class="btn text-white fw-bold shadow-sm px-5 py-3" style="background-color: #f53003; border-radius: 16px; font-size: 1rem;">
                                Publicar en el Foro <i class="bi bi-send-fill ms-2"></i>
                            </button>
                        </div>
                    </form>
                </div>
